<?php 
	$post_tabs_categories_1 = 'Must Do';
	...
	$warrior_set_content .= '<li><a href="javascript:void(0)" data-content="#tab-'.str_replace(" ","",$post_tabs_categories_1).'" title="" class="active">Must Do</a></li>';
	...
	$args_posts_tab_1 = array(
		'post_type' 			=> 'post',
		'post_status' 			=> 'publish',
		'category__and' => array(108,791),
		'post__not_in' => array(32251),
		'posts_per_page' 		=> 3
	);
			//featured post on that category
			$sticky_post_for_this_category = 32251;
			$post_author_id = get_post_field( 'post_author', $sticky_post_for_this_category );	
			$author_name = get_userdata($post_author_id)->display_name;
			$post_date = get_the_date( 'Y-m-d', $sticky_post_for_this_category );
			$single_excerpt = apply_filters('the_excerpt', get_post_field('post_excerpt', $sticky_post_for_this_category));
					...
					if ( has_post_thumbnail() ) {
						$warrior_set_content .= '<a href="'. get_the_permalink($sticky_post_for_this_category) .'" title="'. get_the_title($sticky_post_for_this_category) .'">' .get_the_post_thumbnail( $sticky_post_for_this_category, 'newmagz-large-thumbnail-2' ). '</a>';
					} else {
						$warrior_set_content .= '<a href="'.get_the_permalink($sticky_post_for_this_category).'" title="'. get_the_title($sticky_post_for_this_category) .'">';
						...
						$warrior_set_content .= '<h3 class="post-title"><a href="'. get_the_permalink($sticky_post_for_this_category) .'">'
												.wp_trim_words( get_the_title($sticky_post_for_this_category), absint( $post_tabs_title_limiter ), '...' ).'</a></h3>';
						$warrior_set_content .= '<div class="entry-meta">';
						$warrior_set_content .= '<span class="author"><a href="'.get_author_posts_url( $post_author_id).'">'.$author_name.'</span>';
						$warrior_set_content .= '<span class="date"><i class="fa fa-calendar-minus-o"></i>'.date_i18n( "M j, Y", strtotime($post_date) ).'</span>';
						$warrior_set_content .= '</div>';
						$warrior_set_content .= '<div class="excerpt">';
						$warrior_set_content .= '<p>'.wp_trim_words( $single_excerpt).'</p>';
						$warrior_set_content .= '</div>';
						$warrior_set_content .= '</div>';
			...
			if ( $posts_tab_1->have_posts() ) :
						while( $posts_tab_1->have_posts() ) : $posts_tab_1->the_post();
						$warrior_set_content .= '<div class="column column-2">';			
