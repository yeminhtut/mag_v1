...
if( ! function_exists( 'magazine_dual_author' ) ) {
	function magazine_dual_author() {
		if ( function_exists( 'get_coauthors' ) ) {
		    $authors = get_coauthors();
		    $warrior_set_content = "";	
			$warrior_set_content = '<div class="entry-meta">';	
		    foreach ($authors as $author) {				    	
				$warrior_set_content .= '<span class="author"><a href="'.get_author_posts_url( $author->ID ).'">'.get_avatar( $author->ID, 25 ).$author->display_name.'</a></span>';
		    }
		    $warrior_set_content .= '<span class="date"><i class="fa fa-calendar-minus-o"></i>'.date_i18n( 'M jS, Y', strtotime( get_the_date('Y-m-d'), false ) ).'</span></div>';		    
		}
		else{
			$warrior_set_content = "";
			$warrior_set_content = '<div class="entry-meta" style="margin-bottom:15px !important;">';
...
if( ! function_exists( 'single_author_bio' ) ) {
	function single_author_bio(){
		$authorDescriptionShort = wp_trim_words(get_the_author_meta('description',$author->ID),40,' ');
		$authorDescriptionShort .= '<a href="'.get_author_posts_url( $author->ID ).'">.....</a>';	
        $author_bio_html ='<div class="warrior-tabs-widget article-tabs">
								<div class="widget-title"><h4>About Author</h4></div>
								<div id="about-author" class="tab-content">
									<div class="author-wrapper">
										<div class="thumbnail">'.get_avatar( get_the_author_meta( 'ID' ), 130 ).'</div>
										<div class="detail">
											<h5>'.get_the_author().'</h5>
											<p>'.$authorDescriptionShort.'</p>
											<p style="text-align:center;text-transform:uppercase;font-size:12px;">
												<a href="'.get_author_posts_url( get_the_author_meta( 'ID' ) ).'">CLICK</a> TO SEE MORE ARTICLES BY '.get_the_author().'
											</p>
											<br>
										</div>
									</div>
		...
		return $author_bio_html;
	}
}

if( ! function_exists( 'dual_author_bio' ) ) {
	function dual_author_bio(){
		$authors = get_coauthors();
		$dual_author_bio_heading = 'About Author';
		if (count(get_coauthors())> 1) {
			$dual_author_bio_heading = 'About Authors';
		}
		$author_bio_html = '';
		$author_bio_html .= '<div class="warrior-tabs-widget article-tabs"><div class="widget-title"><h4>'.$dual_author_bio_heading.'</h4></div>';
			foreach ($authors as $author) {
				$authorDescriptionShort = wp_trim_words(get_the_author_meta('description',$author->ID),40,' ');
				$authorDescriptionShort .= '<a href="'.get_author_posts_url( $author->ID ).'">.....</a>';
				$author_bio_html .='<div id="about-author" class="tab-content">
										<div class="author-wrapper">
											<div class="thumbnail">'.get_avatar( $author->ID, 130 ).'</div>
											<div class="detail">
												<h5>'.$author->display_name.'</h5>
												<p>'.$authorDescriptionShort.'</p>
												<p style="text-align:center;text-transform:uppercase;font-size:12px;">
													<a href="'.get_author_posts_url( $author->ID ).'">CLICK</a> TO SEE MORE ARTICLES BY '.$author->display_name.'
												</p>
												<br>
											</div>
										</div>
...
//Impressions
if ( ! function_exists( 'set_banner_impression' ) ) {
	function set_banner_impression($banner_id){
		global $wpdb;
		$banner_id = $banner_id;
		$today = date('Y-m-d');
		$check_banner = $wpdb->get_results("SELECT * FROM `T_Banner_Track` WHERE `BannerID` = $banner_id AND `Date` = '$today'",OBJECT );
		$impression = $check_banner[0]->Impressions;
		if (empty($check_banner)) {
			...
			$current_impression = $check_banner[0]->Impressions;
			$update_impression = $current_impression + 1;
			$wpdb->update( 
				'T_Banner_Track', //table
				array( 
					'Impressions' => $update_impression //data
				), 
				array( 'BannerID' => $banner_id , 'Date' => $today) //where
			);
		}
	}
}
