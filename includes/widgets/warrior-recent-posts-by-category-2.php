<?php
/**
 * Warrior Recent Post by Category (Footer)
 *
 * @package WordPress
 * @subpackage Newmagz
 * @since Newmagz 1.0.0
...
// Widgets
add_action( 'widgets_init', 'warrior_recent_post_by_cat_2_widget' );

// Register our widget
function warrior_recent_post_by_cat_2_widget() {
	register_widget( 'Warrior_Recent_Post_By_Category_2' );
}

// Recent Post by Category Warrior Widget
class Warrior_Recent_Post_By_Category_2 extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
		echo $before_title;
		echo $warrior_recent_post_by_category_title;
		echo $after_title;
?>
		<?php $i = 1; while( $recent_post->have_posts() ) : $recent_post->the_post(); if ($i == 1) : ?>
			...
			if ( $recent_post->post_count > 1 && $i == 2 ) :
			...
			<?php
			if ( $recent_post->post_count > 1 && $i == $recent_post->post_count ) :
				echo '</ul>';
				echo '</div>';
